...
public class MyDBPool {
    private final int poolSize; // 初始化线程池大小

    private Connection[] connections; // 连接对象数组
    private AtomicIntegerArray status; // 连接状态数组，0：空闲，1繁忙
    ...
    public MyDBPool(int poolSize) {
        ...
        for (int i = 0; i < poolSize; i++) {
            connections[i] = new DBConnection("连接" + (i + 1));
        }
    }
    ...
    public Connection borrow() {
        while (true) {
            for (int i = 0; i < poolSize; i++) {
                ...
                if (status.get(i) == 0) {
                    if (status.compareAndSet(i, 0, 1)) {
                        System.out.println(LocalTime.now() + " " + Thread.currentThread().getName() + " borrow " + connections[i]);
            ...
            synchronized (this) {
                try {
                    System.out.println(LocalTime.now() + " " + Thread.currentThread().getName() + " wait...");
                    this.wait();
    ...
    public void free(Connection conn) {
        for (int i = 0; i < poolSize; i++) {
            if (connections[i] == conn) {
                status.set(i, 0);
                synchronized (this) {
                    System.out.println(LocalTime.now() + " " + Thread.currentThread().getName() + " free " + conn);
                    this.notifyAll();
                }
                break;
            }
